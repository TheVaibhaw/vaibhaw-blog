		...
		<?php if (have_posts()) : ?>
			<header class="archive-hero" data-animate="fade-up">
				...
				if (is_category()) {
					...
					$archive_title = is_year() ? get_the_date('Y') : (is_month() ? get_the_date('F Y') : get_the_date());
				} else {
					$archive_title = post_type_archive_title('', false);
				}
				$label = is_category() ? 'Category' : (is_tag() ? 'Tag' : 'Archive');
				?>
				<span class="archive-hero__label"><?php echo esc_html($label); ?></span>
						...
						<a href="<?php echo esc_url(get_permalink()); ?>" class="archive-item__link">
							<div class="archive-item__thumb">
								<?php if (has_post_thumbnail()) : ?>
									<?php the_post_thumbnail('medium', ['class' => 'archive-item__image', 'loading' => 'lazy']); ?>
								<?php else : ?>
